...
router.put('/', checkJwt, async (req, res, next) => {
    const db = getDb();
    try {
        const { tenantId, propertyId, firstName, lastName, email, phoneNumber, observations, contractStartDate, contractEndDate, contractStatus, contractId, contractType, contractValue, contractCurrency, contractPaymentMethod, contractPaymentFrequency, pets, children, smoking } = req.body;
